...
(function (b) {
  b.fn.waitMe = function (p) {
    return this.each(function () {
      var f = b(this),
        ...
        k = {
          init: function () {
            ...
            v = b('<div className="waitMe" data-waitme_id="' + w + '"></div>');
            switch (a.effect) {
              case "none":
                d = 0;
                break;
            ...
            "" !== q && (q += ";");
            if (0 < d) {
              if ("img" === a.effect) r = '<img src="' + a.source + '">';
              else
                for (var e = 1; e <= d; ++e)
                  b.isArray(a.color)
                    ? ((c = a.color[e]), void 0 == c && (c = "#000"))
                    : (c = a.color),
                    (r = u
                      ? r +
                        ('<div className="waitMe_progress_elem' +
                          e +
                          '"><div style="' +
                          t +
                          ":" +
            ...
            a.text &&
              "" === a.maxSize &&
              ((c = b.isArray(a.color) ? a.color[0] : a.color),
              (x = b(
                '<div className="waitMe_text" style="color:' +
                  c +
                  '">' +
                  a.text +
                  "</div>"
            ...
            var h = f.find("> .waitMe");
            h && h.remove();
            c = b('<div className="waitMe_content"></div>');
            c.append(l, x);
            v.append(c);
            ...
            h.css({ background: a.bg });
            m.css({ marginTop: -m.outerHeight() / 2 + "px" });
            "" !== a.maxSize &&
              ((e = l.outerHeight()),
              l.outerWidth(),
              "img" === a.effect
                ? (l.css({ height: a.maxSize + "px" }),
                  l.find(">img").css({ maxHeight: "100%" }),
                  m.css({ marginTop: -m.outerHeight() / 2 + "px" }))
                : a.maxSize < e &&
                  c.css({ transform: "scale(" + a.maxSize / e + ")" }));
            if (f.outerHeight() > b(window).height()) {
              var c = b(window).scrollTop(),
                g = m.outerHeight(),
                n = f.offset().top,
                k = f.outerHeight(),
                e = c - n + b(window).height() / 2;
              0 > e && (e = Math.abs(e));
              0 <= e - g && e + g <= k
                ? n - c > b(window).height() / 2 && (e = g)
                : (e = c > n + k - g ? c - n - g : c - n + g);
              y(e);
      ...
      if (k[p])
        return k[p].apply(this, Array.prototype.slice.call(arguments, 1));
      if ("object" === typeof p || !p) return k.init.apply(this, arguments);
    });
  };
